...
class RetroBridgeDataset(InMemoryDataset):
    ...
    def __init__(self, stage, root, extra_nodes=False, swap=False, 
                 retrieval_dataset=None, augmented_graphfeature=False, use_cluster=False,):
        self.stage = stage
    ...
    def raw_file_names(self):
        if self.use_cluster:
            return ['uspto50k_train_scaffold.csv', 'uspto50k_val_scaffold.csv', 'uspto50k_test_scaffold.csv']
        elif self.retrieval_dataset == '50k':
    ...
    def processed_file_names(self):
        if self.use_cluster:
            return [f'train_scaffold.pt', f'val_scaffold.pt', f'test_scaffold.pt']
        elif self.retrieval_dataset == '50k':
    ...
    def process(self):
        ...
        for i, row in tqdm(table.iterrows(), total=table.shape[0]):
            retrieval_list = torch.tensor([int(x) for x in row['retrieval_index'].split(',')], dtype=torch.long).unsqueeze(0)
            #print(retrieval_list.shape)
            #retrieval_list = torch.tensor([row['retrieval_index']], dtype=torch.long).unsqueeze(0)
...
class RetroBridgeDataModule(MolecularDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        datasets = {
            'train': self.DATASET_CLASS(stage=stage, root=self.data_root, extra_nodes=self.extra_nodes, swap=self.swap,retrieval_dataset=self.retrieval_dataset, augmented_graphfeature=self.augmented_graphfeature, use_cluster=self.use_cluster),
            'val': self.DATASET_CLASS(stage='val', root=self.data_root, extra_nodes=self.extra_nodes, swap=self.swap,retrieval_dataset=self.retrieval_dataset, augmented_graphfeature=self.augmented_graphfeature, use_cluster=self.use_cluster),
            'test': self.DATASET_CLASS(stage='test', root=self.data_root, extra_nodes=self.extra_nodes, swap=self.swap,retrieval_dataset=self.retrieval_dataset, augmented_graphfeature=self.augmented_graphfeature, use_cluster=self.use_cluster),
        }
