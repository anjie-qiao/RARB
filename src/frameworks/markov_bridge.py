...
class MarkovBridge(pl.LightningModule):
    ...
    def process_and_forward(self, data):
        ...
        if self.retrieval_k > 0:
            ...
            random_indices_expanded = random_indices.unsqueeze(-1).expand(retrieval_emb.size(0), self.retrieval_k, retrieval_emb.size(-1)).to(self.device)
    ...
    def my2_compute_p_zs_given_p_zt(self, z_t, pred, node_mask, t):
        ...
        X_T_all = F.one_hot(torch.arange(self.Xdim_output, device=self.device), num_classes=self.Xdim_output).float()
        E_T_all = F.one_hot(torch.arange(self.Edim_output, device=self.device), num_classes=self.Edim_output).float()
    ...
    def my_compute_p_zs_given_p_zt(self, z_t, pred, node_mask, t):
        ...
        X_T_i_e = F.one_hot(torch.zeros_like(p_X_T[..., 0]).long(), num_classes=self.Xdim_output).float()
        for i in range(self.Edim_output):
            z_T = utils.PlaceHolder(X_T_i_e,E_T_all[i])
            _, prob_E_i = self.compute_q_zs_given_q_zt(z_t, z_T, node_mask, t)  # bs, n, n, d
            prob_E += prob_E_i * p_E_T[..., i].unsqueeze(-1)  # bs, n, n, d
    ...
    def my_compute_p_zs_given_p_zt(self, z_t, pred, node_mask, t):
        ...
        for i in range(4):
            ...
            p_E_T = F.gumbel_softmax(pred.E, tau=1.0, hard=True)
