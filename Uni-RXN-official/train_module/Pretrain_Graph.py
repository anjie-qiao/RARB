...
class Pretrain_Graph(LightningModule):
    ...
    def generate_reaction_fp_mix(self, reaction_rxn, device, no_reagent=False):
        ...
        product_mol = Chem.MolFromSmiles(reaction_rxn[2])
        ...
        reactant_fp, reactant_item = self.generate_reactant_fp(reactant_mol, device)
        product_fp, product_item = self.generate_product_fp(product_mol, device)
        ...
        if not no_reagent:
            ...
            return torch.cat([torch.sum(reactant_fp[:, 0, :], dim=0, keepdim=True), reagent_fp], dim=1), torch.sum(product_fp[:, 0, :], dim=0, keepdim=True)
        else:
            return torch.sum(reactant_fp[:, 0, :], dim=0, keepdim=True), torch.sum(product_fp[:, 0, :], dim=0, keepdim=True)
